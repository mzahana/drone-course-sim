#!/usr/bin/env python3
"""Lab 3 skeleton -- arm, OFFBOARD, take off, fly a square, then
...
class OffboardSquare(Node):
    ...
    def _send_position(self, x, y, z, yaw=0.0):
        # TODO(student): fill in a PositionTarget and publish it on self.sp.
        # coordinate_frame must be PositionTarget.FRAME_LOCAL_NED
    ...
    def _send_velocity(self, vx, vy, vz, yaw=0.0):
        # TODO(student): the same, but with VELOCITY_ONLY and the velocity fields.
        raise NotImplementedError

    def _corner_xy(self, i):
        # TODO(student): return the (x, y) of corner i of a square of side self.side,
        # starting from self.home. i wraps with % 4.
        raise NotImplementedError
    ...
    def _tick(self):
        ...
        if self.phase == "STREAM":
            ...
            self._send_position(self.home[0], self.home[1], self.alt)
            # TODO(student): ask for OFFBOARD, then arm, then move on to TAKEOFF.
            # Order matters: PX4 will not accept OFFBOARD unless
